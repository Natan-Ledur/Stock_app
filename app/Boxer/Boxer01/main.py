...
# Adjust MONGO_URI when running locally on Windows: inside Docker the host
# `mongodb2:27017` is valid, but when running the script on Windows the name
# won't resolve. Prefer `MONGO_URI_COMPASS` if provided, otherwise replace
# the container hostname with localhost:27018 when on Windows.
import platform
try:
    if platform.system().lower().startswith('windows'):
        # prefer explicit COMPASS uri if present
        compass = os.environ.get('MONGO_URI_COMPASS')
        if compass:
            MONGO_URI = compass
        else:
            # naive replacement if the env still contains mongodb2
            if 'mongodb2:27017' in MONGO_URI:
                MONGO_URI = MONGO_URI.replace('mongodb2:27017', 'localhost:27018')

    # Create client with a short serverSelectionTimeoutMS to fail fast locally
    try:
        client = MongoClient(MONGO_URI, serverSelectionTimeoutMS=5000)
        # force server selection now
