...
###====================================================================consumo boxer + estoque - finte sisplam===============================================================================
...
# 1) remover duplicatas exatas (todas as colunas)
df = df_et01.drop_duplicates().copy()

# 2) transformar 'U' em lista de faixas e explodir
df['faixa'] = df['faixa'].apply(lambda x: ['P','M','G','GG'] if x == 'U' else x)
df = df.explode('faixa')

# 3) remover duplicatas de novo (caso a expansão crie duplicatas) e resetar índice
df = df.drop_duplicates().reset_index(drop=True)

# substituir o df original se quiser
df_et01 = df
...
df_merged['quantidade_final'] = ((df_merged['qtde'] + df_merged['qtde_fat']) * df_merged['categoria'] ) * (df_merged['consumo'] / df_merged['soma_consumos'].replace(0, 1))
