...
def _get_mongo_client_and_db():
    """Estabelece conexão com o MongoDB baseada no ambiente."""
    mongo_uri = os.environ.get("MONGO_URI")
    mongo_db = os.environ.get("MONGO_DB", "mydb")
    is_windows = platform.system() == 'Windows'

    # Prioridade para URI do Compass no Windows
    if is_windows:
        compass_uri = os.environ.get('MONGO_URI_COMPASS')
        if compass_uri:
            mongo_uri = compass_uri
        elif mongo_uri and 'mongodb2:27017' in mongo_uri:
    ...
    if not mongo_uri:
        ...
        mongo_uri = f"mongodb://{user}:{pw}@{host}:27017/"
    ...
    try:
        ...
        logging.warning(f"Erro ao conectar no MongoDB ({mongo_uri}): {e}")
        return None, None
